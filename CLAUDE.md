...
- `scrape_vinolist.py` + `parse_vinolist.py` + `compile_vinolist_signals.py` + `build_vinolist_rollups.py` — **planned** Vinolist NYC pipeline (NYC restaurant wine lists → per-producer `community.vinolist.*` signals + rollups). Not yet written; source layer + data contract scaffolded in `raw/vinolist/` (manual-paste ingest for now — vinolistnyc.com 403s the fetcher + host egress-blocked).
