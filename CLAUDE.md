...
- **Wine-list monitor — first snapshots** — pipeline scaffolded (2026-05-27, scripts/scrape_wine_lists.py + parse + diff + build_wine_list_view, plus `.github/workflows/wine_lists.yml` weekly cron). v1 sources: Chambers pours, Estela, Peasant, Claud, Noreetuh. The workflow runs Mondays 14:00 UTC and opens a PR with the week's snapshots — first run will land the initial snapshot per source (no diff yet); the week-2 run produces the first arrivals view. Trigger ad-hoc via `workflow_dispatch` on GitHub Actions UI. Per-source parsers in `parse_wine_list.py::PARSERS` are starter implementations using the generic line-and-section parser — once the first PRs land, eyeball the snapshot.json output and tune the per-source functions if extraction is dropping fields or splitting cuvées wrong. v2 backlog: Terroir Tribeca, Dame, Café Altro Paradiso, Saint Urban (deferred, need OCR or no public list URL).
