...
- **Cellar triage — revisit bucket** — the 2026-06-09 triage (`scripts/compile_cellar_triage.py`, decision record in `wiki/_views/cellar_triage_2026_06.md`) created 58 pages (56 cellar keepers + Dunn/Corison reference pages), recorded 50 deliberate no-page decisions, and parked **54 "revisit" producers** for a pass with Evan (Beaujolais/Loire/Piedmont next-pass names, Portugal/Switzerland taxonomy questions, Casa Dumetz/Clementine Carter pair, grant_marie slug dedup). Review the revisit table when adding coverage in those regions.
- **Architecture follow-ups** — ranked in `wiki/_views/architecture_review_2026_06.md`: `updated:`/price `as_of:` freshness fields, `scripts/lib/` extraction + pytest idempotency suite + `--check` for rollups, Obsidian Bases live drink-window view, lateral "Neighbors" blocks on producer pages. (`status: stub|seeded|curated` is now in `_SCHEMA.md`; backfill legacy pages opportunistically.)
