...
- **Critic-ratings pipeline** (`## Critic Ratings` section; schema in `_SCHEMA.md`). Multiple landing sources → one shared compiler:
  - `parse_auction_ratings.py` — critic scores + note excerpts embedded in an auction catalog's `WineNote` column → `raw/ratings/<sale>/ratings_week<n>.json` (261W W30 = 784 scored lots).
