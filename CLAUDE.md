...
- `parse_auction_ratings.py` + `compile_auction_ratings.py` — **auction critic-ratings pipeline**. `parse_` lands the critic scores + note excerpts embedded in an auction catalog's `WineNote` column to `raw/ratings/<sale>/ratings_week<n>.json`; `compile_` matches them to producer pages (conservative name matching; unmatched → `build/auction_ratings_report.md`) and writes a `## Critic Ratings` table (schema in `_SCHEMA.md`). Idempotent, `--apply` to write. First run: 261W W30 → 784 scored lots, 53 producers. Re-run `compile_` after adding producer pages to pick up previously-unmatched ratings.
