...
- `compile_clippings.py` — Vinous / Wine Advocate (Kelley) *article* clippings → `## Vinous Reviews` / `## Wine Advocate (Kelley)` sections (drop clippings in `raw/clippings/<source>/`).
- **Critic-ratings pipeline** (`## Critic Ratings` section; schema in `_SCHEMA.md`). Multiple landing sources → one shared compiler:
  - `parse_auction_ratings.py` — critic scores + note excerpts embedded in an auction catalog's `WineNote` column → `raw/ratings/<sale>/ratings_week<n>.json`. **Parses ALL catalogs in `raw/auctions/` by default** (not just the newest — that older behavior silently skipped every week but the last). `--check` gates CI: fails if any catalog is unlanded. Current landing: 7 catalogs (261W weeks 25/26/28/29/30 + a June snapshot, plus the 265DE sale) = 5,627 scored lots. 261W is a rolling weekly sale (lots persist week to week); the compile dedupes by (wine, vintage, critic, score) so overlaps collapse. Old `.xls` (Zachys) is skipped as a landing gap — needs conversion to `.xlsx`.
  - `parse_raeders_ratings.py` — the four Raeders score columns (`score_wa`/`score_js`/`score_w_s`/`score_we`) + `tasting_note` → `raw/ratings/raeders/ratings_<date>.json` (2026-04-25 snapshot = 1,038 scored wines / 2,174 score records; each record carries an explicit `producer_slug`).
  - `compile_auction_ratings.py` — reads **all** `raw/ratings/**/ratings*.json`, matches to producer pages (page-backed `producer_slug` first, else conservative name matching with an ambiguity guard; unmatched → `build/auction_ratings_report.md`), and writes one merged `## Critic Ratings` table per producer (Source column distinguishes auction lot vs Raeders snapshot). Idempotent, `--apply`. Current: 121 producers matched across all 7 catalogs. Re-run after adding producer pages or aliases to pick up previously-unmatched ratings; unmatched lots (incl. multi-word southern-Italian names like "Dal Forno", "Azienda Vinicola Contini", and page-less producers like Galardi/Montevetrano) accumulate in `build/auction_ratings_report.md` for review. **Next sources:** Raeders is done; the wired-but-empty Vinous/WA *article* clippings (`compile_clippings.py`) remain manual-paste.
