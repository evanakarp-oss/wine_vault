...
## High-conviction buy list — on-taste, from a trusted source, not owned (213)
...
## Tier-1 trusted-source producers (316)
...
| [[beck-hartweg\|Beck-Hartweg]] | Alsace · Dambach-la-Ville (Bas-Rhin) | core | chambers | — |
| [[beck_hartweg\|Beck-Hartweg]] | Alsace · Pfaffenheim | core | Jenny & François, chambers | 🍷 |
...
The join is only as good as the fields it reads. Coverage across 829 producers (raise these to sharpen every future answer):

- **importer_us (trust source): 187 (22%)** — the single highest-leverage backfill; every unfilled one is a producer whose trust signal is invisible.
- **appellations (cru / vineyard 'land'): 274 (33%)** — the gap blocking cru-level 'producer × land' reasoning (who else works Les Chaignots / Howell Mtn). Backfill from the cuvée names already sitting in `## Critic Ratings` + the DTE cuvée tables.
- **farming (style): 304 (36%)**.
- **trust_tier resolved: 377 (45%)**.

## Link integrity (graph edges)

**911 distinct broken `[[wikilink]]` targets** (2042 occurrences) across wiki + _views + cellar — edges that point at nothing, so multi-hop traversal dead-ends. Top offenders and the fix plan are in [[data_quality_integration_review_2026_07]] (rec #2). Making `lint.py` validate every target turns this into a build-time gate.
