...
## [2026-06-05] ingest | L'If producer stub + Thienpont family map (from /ask-cellar thread)

Created `wiki/producers/l_if.md` from an `/ask-cellar` Q&A on L'If and the 2011 right bank. L'If is **Saint-Émilion** (not Pomerol, as the question framed it) — Jacques Thienpont's St-Émilion sibling to Pomerol's Le Pin. Captured the Thienpont-family map: **Jacques** = Le Pin + L'If + L'Hêtre; **Alexandre** (+ son **Guillaume**) = Vieux Château Certan; Guillaume also at Guillot-Clauzel. L'If and VCC are cousins' estates, two branches — commonly conflated. Person↔estate pairings corroborated in the WK Berserkers corpus; cousin tree is established Bordeaux fact. Also folded in WK's bullish 2011-right-bank value view (VCC/Cheval Blanc 2011 > 2014; "2011 VCC… value pick of the decade"). No WA clipping exists and WK doesn't post on L'If himself — the corpus L'If chatter is other forum users. Page is a stub: no CSW/Raeder's/DTE/FASS presence, farming left empty (unconfirmed). Re-ran `build_wiki_index.py`. Lint exits 0; two `broken_wikilink` backlog items remain (`le_pin`, `vieux_chateau_certan`) — left as "pages to create" rather than auto-spawned, since cult right-bank Pomerol may sit outside Evan's curation taste (his call). See Open follow-ups.
