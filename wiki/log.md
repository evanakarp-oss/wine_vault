...
## [2026-05-30] ingest | Fass Selections inventory: +153 producer pages + Brisset, every page with deduped ## FASS section

Ingested the Fass Selections Spring 2025 inventory (`Fass File.xlsx` → Cleaned Inventory, 1,253 rows). New script `scripts/compile_fass_creates.py` (idempotent LLM-judgment pass): canonicalizes producers (72-entry lookup + manual merges + stopword-stripped group key), drops corrupt "shifted" source rows (producer name split across producer/wine columns, year misread into price), resolves country/region against `_TAXONOMY.md`, dedupes cuvées (collapses vintages → price range), and pulls editorial from the 180 `raw/fass/markdown` articles where a title matches (~30 pages got a `## Fass Notes` excerpt). Created **153** new producer pages + the explicitly-requested **Maison Pierre Brisset** (16 bottles owned, was a cellar-producer gap). Merged Fass data into 3 existing surname-stub pages (Steinmetz←Günther Steinmetz, Billon←Christophe Billon, Jasmin←Patrick Jasmin) rather than duplicate. Every Fass producer now carries a deduplicated `## FASS` cuvée table + `retailers.fass` frontmatter, so `/ask-cellar "what does Fass sell from X"` resolves directly. Producers 361→514. Taxonomy extended: Switzerland (country + 8 regions), US regions (California/Oregon/Washington/New York), Italy +Molise/Umbria. **Held (4, need manual triage):** Bellaria & Vigneti Costacurta (region unresolvable from source), Cuchet-Beliando Cornas (stray variant), E. Guigal (all prices `######` Excel column-overflow). Reports: `build/fass_creates_report.md`, `build/fass_candidates.md`. Rollups + index regenerated; lint 0, index/log `--check` pass.
