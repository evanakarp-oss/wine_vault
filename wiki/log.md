...
## [2026-05-30] ingest | FASS offers ingested into 33 producer pages (+ date-extractor fix)

Ran the FASS offer pipeline end to end. (1) Patched `scrape_blogs.py` date extraction — fassselections.com (Divi theme) carries no `<time>`/published_time meta; the date lives in `<span class="published">Nov 6, 2023</span>` and the byline regex missed it (author `admin` wrapped in `<a>`). Added a `span.published` selector. `--reparse` over the 181 cached HTML files backfilled dates on all 181 (previously 0 dated). (2) `ingest_blog_articles.py --source fass`: scanned 181 FASS offer posts against 360 producer pages, matched **33** producers, wrote `## FASS Write-ups` sections (★ = title hit) + `fass.{championed,article_count,dedicated_count,first_year,last_year}` frontmatter. Top: Steinmetz (29 offers/5★), Enderle & Moll (23/8), Sven Enderle (15/5), François Buffet (13), Allemand/Billon/Gripa (5 each). 324 no-match (mostly Bordeaux/Argentina/Champagne names outside the FASS Burgundy-Mosel-Rhône-Piedmont core); 3 skipped (Gaja/Roty/Tutu — names too short for safe whole-word matching). Report: `build/fass_ingest_report.md`. Followup: **Maison Pierre Brisset has no producer page** (7 cuvées in cellar, multiple FASS offers incl. the 2022 Cassaneas) so his offers landed nowhere — seed the page to capture them. Re-run `ingest_blog_articles.py --source fass` after.

## [2026-05-30] ingest | Maison Pierre Brisset producer page seeded (FASS curation)

Created `wiki/producers/maison_pierre_brisset.md` — the Brisset gap flagged in the prior FASS ingest. He's a Lyle Fass micro-négociant (Côte de Beaune/Nuits) and one of Fass's house benchmarks; 7 cuvées / 16 bottles in cellar. **Hand-curated, not auto-ingested**: running `ingest_blog_articles.py --source fass` with a `"Brisset"` alias matched 29 articles but only ONE is a real Brisset offer (the 2022 "Cuvee Cassaneas," 9.6, ~$40/case). The other 28 — including 3 of 4 ★ title-hits — are Fass using "Brisset" as a comparison benchmark in *other* producers' offers ("If Brisset Made Nebbiolo" = Ar.Pe.Pe; "if Brisset and Buffet Had a Love Child" = Thörle; "Closest Wines to Brisset" = grower Champagne). Textbook CLAUDE.md anti-pattern (retailer-source matching needs LLM judgment). Resolution: `aliases: []` (canonical "Maison Pierre Brisset" appears in 0 article bodies, so future `--source fass` passes won't re-pollute), curated `## FASS` lineup (3 whites incl. Abbaye de Morgeot 9.9 / St-Aubin Sur Roche Dumay 9.9 / Cassaneas 9.6; reds Pommard/NSG/Chambolle/Vosne 1ers + Gabrius), single real offer in `## FASS Write-ups`, and a note documenting the benchmark name-drops. fass ingest dry-run reconfirmed at 33 matched (Brisset correctly excluded). lint 0, index/log --check pass.
