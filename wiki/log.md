...
## [2026-05-26] ingest | erwine_portraits: 76 new producer pages from E & R Wine Shop (Portland)

Ingested E & R Wine Shop's "Producer Portraits" series (https://www.erwineshop.com/producer-portraits) into the vault. Source is the long-running editorial program from the Portland, OR retailer (Ed Paladino + team) — first-person write-ups of growers visited in their cellars, anchored by the shop's Almost Direct Imports program.

**Created 76 new producer pages.** Geography: Alsace (4: Loew, Marcel Deiss, Zusslin, Mader), Burgundy (6: Béru, Raphaëlle Guyot, Les Faverelles, Sangouard-Guyot, Michel Lafarge, Fèvre-Chablis), Champagne (7 grower: Suenen, Christophe Mignon, Pascal Doquet, Lahaye, De Sousa, Eric Rodez, Franck Pascal), Beaujolais (Duport, Chemarin), Rhône (Chave), Jura (Cellier St Benoît, Fumey-Chatelain), Loire (Florian Roblin, La Grange Tiphaine), SW France (Issaly/La Ramaye), Italy 30+ (Piedmont: Prandi, Burzi, Correggia, Cecilia Monte; Valle d'Aosta: Lo Triolet/Marco Martin, La Vrille; Lombardy: Mamete Prevostini, Terrazzi Alti; Alto Adige: Kuen Hof, Ebner, Franz Gojer, Thomas Niedermayr; Veneto: Menti; Friuli: Dario Raccaro; Emilia-Romagna: Paltrinieri, Della Volta, Villa Picta; Liguria/Cinque Terre: Prima Terra/De Battè, Litan, La Possa, Cian du Giorgi; Tuscany: Baricci, San Polino; Marche: Coroncino, Ca'Liptra, Fiorano, La Monacesca; Abruzzo: Faraone; Lazio: Damiano Ciolli, La Visciola; Campania: I Cacciagalli, Pietracupa, Azienda Reale; Sicily: I Vigneri/Salvo Foti), Oregon 17 (Willamette: Grape Ink, Crowley, Granville, Kelley Fox, Liska, Walter Scott, Niew, Lingua Franca, Shiba-Wichern, Forrest Schaad, Bethel Heights, Hundred Suns, Junichi Fujita, Art + Science, Gonzales; Columbia Gorge: Analemma, Loop de Loop). **Overlay** added `## E&R Wine Shop` section to 4 existing pages (`domaine_bart`, `domaine_chantereves`, `elio_ottin`, `marguet`).

**Schema changes.** Added `### United States` region section to `wiki/_TAXONOMY.md` (Oregon, California, Washington, Columbia Gorge) so Oregon-anchored E&R producers don't fail lint. Added `erwine` retailer block to new producer frontmatter (`in_portfolio`, `portrait_url`); not yet a first-class retailer key in `_TAXONOMY.md` — left as an additive convention pending broader retailer block work.

**Surfaces.** Created `wiki/retailers/E_R_Wine_Shop.md`. Build_rollups generated new `wiki/regions/Oregon_Producers.md` (15 producers) and `wiki/regions/Columbia_Gorge_Producers.md` (2). Producer count: 360 → 436.

**Source caveat.** WebFetch returned 403 from erwineshop.com and the host wasn't on the sandbox curl allowlist; enumeration was done via Google snippets (WebSearch), so per-producer summaries are short editorial excerpts from snippets rather than the full portrait bodies. The canonical portrait URL is captured in frontmatter (`retailers.erwine.portrait_url`) and the `## E&R Wine Shop` body section, so a future pass can deepen each page from the live source when the site is reachable. Code: `scripts/ingest_erwine_portraits.py` (idempotent — re-running skips existing pages; existing-overlay block uses `<!-- BEGIN/END E&R WINE SHOP -->` sentinels so it can be regenerated in place).
