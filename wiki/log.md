...
## [2026-06-09] fix | Architecture pass: cellar↔wiki linking, HOME dashboard, broken-link repair, country hubs

Full-vault audit (filed as `wiki/_views/architecture_review_2026_06.md`) followed by a navigation fix pass. (1) New `scripts/link_cellar.py`: curated slug-override table mapping 13 CellarTracker-derived slugs to canonical wiki slugs (verified by hand; rejected false positives like Oregon's Vincent ≠ vincent_paris) — 30 cellar files re-slugged/renamed, 51 cellar bodies now wikilink their producer, 27 producer pages got `## Cellar` sections with per-bottle wikilinks. `ingest_cellar.py` applies the same overrides on re-ingest. (2) New `scripts/fix_crossrefs.py`: 137 producer pages repaired — `[[CSW Article Archive]]` re-targeted to Chambers_Street_Wines, accent variants fixed (Rhône→Rhone), 142 location links unwrapped to plain text (policy: sub-regions stay plain text until a page exists). (3) `build_rollups.py` emits 6 country hubs (France/Italy/Germany/Argentina/Spain/United_States `_Producers` region directories) that Cross-references already pointed at. (4) New `wiki/HOME.md` via `scripts/build_home.py` (--check-able): drink-window urgency counts, saved-analyses table, browse-by-country/region, cellar stats. `wiki/_views/` is now indexed in `index.md` (was in the generator's skip list — 8 analyses had zero inbound links). (5) Generators (`build_views.py`, `build_wiki_index.py`) only wikilink producers that have pages; drink-window rows link the cellar bottle file. `lint.py` now validates slug-shaped wikilinks against all wiki+cellar stems. (6) Data repairs: arnot-roberts France/Jura → United States/California (Healdsburg); United States region section added to `_TAXONOMY.md`; nested `wiki/.obsidian/` deleted (vault root = repo root, documented in WORKFLOW.md). Net: broken wikilink occurrences 764 → ~10. Follow-ups ranked in the review view; biggest: 160 cellar producers still without wiki pages (incl. Dunn + Corison) need a taste-filter triage.

## [2026-06-09] compile | Cellar producer triage: 58 pages created, 50 no-page, 54 revisit

LLM-judgment pass over the 160 cellar producers without wiki pages, applying the CLAUDE.md taste filters (`scripts/compile_cellar_triage.py`, decision table in code; full record in `wiki/_views/cellar_triage_2026_06.md`). Created 56 seeded producer pages (`status: seeded`, now documented in `_SCHEMA.md`) — highlights: Ceritas, ARPEPE, Ronchi di Cialla, Radikon, Dario Prinčič, Marcel Lapierre, Jean Foillard, J.-F. Mugnier, Armand Rousseau, Château Guiraud, A. Christmann, Julian Haart, Ziereisen, Envínate, Franz Hirtzberger. Plus two taste-filter reference pages NOT from the cellar: Dunn Vineyards and Corison (erratum: the previous log entry claimed they were cellar holdings — they were never in the cellar; they were missing from the wiki entirely). Recorded 50 deliberate no-page decisions (cult-hedonist Napa per filter: Shafer/Bond/Marcassin/Andremily; négoce-generic: Bouchard, Drouhin, Antinori, de Négoce; one-offs) and 54 revisit (next-pass keepers + Portugal/Switzerland taxonomy questions + grant_marie slug dedup). Ran `ingest_csw.py` after creation: 25 of the new pages matched existing CSW coverage (Lapierre 30 articles, Foillard 19, Envínate 15, Rousseau 9, Corison 4, Dunn 2); the sweep also repaired a long-standing false-positive — `vincent_paris.md` had `aliases: ["Paris"]`, matching the city in ~48 articles (alias removed, count back to 2) — and picked up slash-name matches for existing pages (Peter Lauer 0→24, Vignes du Maynes 0→21, Lafouge 0→13). Taxonomy additions: Italy +Trentino +Umbria, Spain +Canary Islands, Austria section, Czech Republic country+regions. Cellar producers without pages: 160 → 104, all with recorded decisions. Vault: 360 → 418 producer pages, 46 region rollups, 8 country hubs.
