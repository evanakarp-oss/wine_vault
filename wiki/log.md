...
## [2026-06-07] update | Fill Lucien Boillot stub — Kermit Lynch importer + farming + editorial

Filled the bare Raeders-seeded `wiki/producers/lucien_boillot.md` stub. Verified via Kermit Lynch that Domaine Lucien Boillot et Fils (Gevrey; run by Pierre Boillot, son of namesake Lucien; holdings across Gevrey/NSG/Volnay/Pommard) is a Kermit Lynch US import — set importer_us, regenerated the Kermit Lynch importer rollup (now lists the producer). Set farming: sustainable (lutte raisonnée / organic-inspired, not certified organic). Wrote editorial summary; flagged the name-traps (WK's "Boillot" comments are all Henri Boillot, not Lucien; the celebrated Pommard "Les Fremiers" is Thomas Bouley's, not Boillot's) and the brother domaine Louis Boillot (CSW value write-up). Source: kermitlynch.com, verified 2026-06-07.
