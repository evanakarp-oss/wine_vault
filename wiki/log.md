...
## [2026-06-21] view | What William Kelley (WK) says he likes — palate + Bordeaux overlay
Mined the in-vault William Kelley Berserkers corpus (4,727 of his own posts, raw/berserkers/William_Kelley/) plus public web to answer "what does WK say he likes?" Filed `wiki/_views/william_kelley_taste_2026_06.md`: his aesthetic (tension/freshness/dry-extract over ripeness+oak; farming as "the missing link"; results-not-certificates on organic/biodynamic; false modern-vs-traditional binary; maturity over hierarchy) and Bordeaux worldview ("contemporary classicism — best since the early 1960s"; value beyond famous labels; "back to DNA" estates Canon/Rauzan-Ségla/Giscours/Gruaud/Troplong). Noted the formal Wine Advocate score clippings never landed (raw/clippings/wine_advocate/ is README-only) — answer built from forum prose. Added a WK overlay + per-producer verdict table to `flatiron_bordeaux_sale_2026_06.md`, correcting two picks: Pontet-Canet 2007 is on the right side of WK's ≤2011 line (he's critical of 2012+), and added Cantemerle 2019 ($39.99) as WK-endorsed value. Views index 80 → 81.

## [2026-06-21] ingest | Seed William Kelley editorial takes on 6 Bordeaux producer pages
Added a `## Notes` → `### William Kelley — take` section to chateau_haut_bailly, chateau_cantemerle, chateau_gruaud_larose, chateau_leoville_barton, domaine_de_chevalier, and chateau_rauzan_segla — a synthesized WK verdict per estate (value vintages, drink windows, "back to DNA" status) with one verified verbatim pull-quote + WB post URL each. Quotes cross-checked against the raw William_Kelley corpus (raw/berserkers/William_Kelley/markdown/). Deliberately did NOT fill the empty `## Wine Advocate (Kelley)` slot: per _SCHEMA.md that section is for publication clippings (raw/clippings/wine_advocate/), and the source here is his forum prose — mislabeling would conflate community vs editorial signal. Gap: Giscours, Pontet-Canet, du Tertre, Potensac have strong WK takes but no producer pages yet.
