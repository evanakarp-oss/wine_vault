...
## [2026-05-26] ingest | fass: full portfolio rebuild from fresh TSV dumps

Replaced the broken FASS state (4 producers with $0 prices because the JSX seed was Windows-only) with a clean rebuild driven by `raw/fass/portfolio_2026-05-26.tsv` (1,166 rows, bulk dump) and `raw/fass/portfolio_curated_2026-05-26.tsv` (300 rows, color/variety/appellation overlay). Stripped FASS state from all 360 producer pages, then re-wrote `## FASS` sections and `retailers.fass:` frontmatter blocks on 14 matched producers (Steinmetz 41 cuvées, François Buffet 33, Michel Rebourgeon 25, Enderle & Moll 18, Sven Enderle 18, Domaine Gripa 14, Billon 6, Jasmin 6, Peter Lauer 4, Pavelot 3, Pierre Gonon 3, Weiser-Künstler 3, Dönnhoff 2, Vincent Paris 1). Real prices now populated. 227 unmatched producers logged to `build/fass_ingest_report.md` for triage — top candidates by cuvée count (Markus Molitor 35, Pierre Brisset 26, Thörle 24, Maison Brisset 22, Richard Östreicher 21, JJ Girard 20, Cesare Bussolo 19, Julien Barge 19, Domaine des Roches Neuves 18, Chavy-Chouet 17) are the obvious next onboarding sweep. Rewrote `scripts/ingest_fass.py` to read TSV (was JSX on Windows path), added prefix/suffix-stripping slug heuristic, and expanded `FASS_ALIASES`. Idempotent: re-running produces identical vault state. Run: `scripts/ingest_fass.py` → `scripts/build_rollups.py` → `scripts/build_wiki_index.py`. Lint: 0 issues.

## [2026-05-26] view | fass_unmatched_triage_2026_05: triage of 227 FASS unmatched into onboard/watch/skip

Filed `wiki/_views/fass_unmatched_triage_2026_05.md`. Triages the 227 unmatched FASS producers (collapsing to ~110 distinct after alias-merging spelling/typo variants) into 4 buckets per Evan's curation filters: Tier 1 onboard ~50 producers (Markus Molitor, Pierre Brisset, Chavy-Chouet, Domaine des Roches Neuves, ARPEPE, Chateau Thivin, Vietti, Chateau Simone, Gut Hermannsberg, Dr Bürklin-Wolf, Ziereisen, Bel Air-Marquis d'Aligre, Yvon Métras, Jean Foillard, Marc Hébrart, Markus Molitor, JJ Prüm, Selbach-Oster, Cesare Bussolo, Julien Barge, the Cornas cluster (Cuchet-Beliando/Guillaume Gilles/Domaine du Tunnel/Jacques Lemenicier/Emmanuel Verset/Mickael Bourg/Compagnie de l'Hermitage), the Chablis cluster (Dauvissat/Tribut/Dampt/Defaix), the Champagne grower cluster, the Graubünden Pinot cluster (Adank/Mohr-Niggli/Patrick Adank/Sprecher von Bernegg), Caves Jean Bourdy, Gilles Berlioz); Tier 2 ~20 producers worth research (Franken cluster Östreicher/Walter/Weltner, Baden Achim Dürr/Von der Mark, Swiss minor, Italian South Cianfagna/Vini Marino/Calafe); Tier 3 ~40 skip (generic mid-tier Bordeaux Chateau Bouillerot/Samion/Bonneau/Vieux St-Andree, single-bottle obscure, broken split-row data); 25 alias-only spelling dupes ready to merge once target pages exist. Provides FASS_ALIASES code block for the ingest script and a 5-step recommended onboarding sequence. Open questions for Evan: Bel Air-Marquis d'Aligre (long-élevage aged Cru Bourgeois) — keep or skip per "no generic mid-tier"? Cahors / Jouffreau — accept Southwest France as covered region? Franken — open the region or stay focused on the core German regions?

## [2026-05-26] ingest | fass: Tier 1 sweep — 109 new producer pages onboarded

Seeded **109 new producer pages** for the FASS Tier 1 list curated in `wiki/_views/fass_unmatched_triage_2026_05.md`. New `scripts/seed_fass_tier1.py` is the canonical one-shot writer: full frontmatter per `_SCHEMA.md` (country, region, sub_region, appellations, farming, certifications, importer_us, retailers stub, tags, _sources=fass_tsv) + a 50-200-word producer summary written from training knowledge. After seeding, ran `ingest_csw.py` (218 producers updated with CSW article matches), then `ingest_fass.py` with an expanded `FASS_ALIASES` block consolidating all the spelling/typo variants from the triage. Result: FASS retailer rollup now has **123 producers** (was 14). Region breadth gained Switzerland (4 Graubünden producers), Franken (Östreicher / Walter / Weltner), South West / Cahors (Famille Jouffreau), and broader Alto Piemonte (Boca, Ghemme, Sizzano, Fara, Bramaterra, Carema). Taxonomy patched to add Switzerland (country + Graubünden / Eastern Switzerland / Basel-Land / Vully regions) and United States regions (Oregon / California / New York / Washington). Lint: 0 issues. Outstanding: ~30 Tier 2 producers (Franken-or-not borderline, Italian South Cilento/Sicily, Swiss minor) and the remaining alias-only spelling variants in `build/fass_ingest_report.md`.

## [2026-05-26] view | fass_compelling_picks_2026_05: signal-tested FASS picks (WB + WK + curation)

Filed `wiki/_views/fass_compelling_picks_2026_05.md`. Strips FASS marketing tone from the 123-producer rebuilt portfolio and tests each pick against WB top-100 (1,089 posts), William Kelley's 4,729 Berserkers posts, and CLAUDE.md curation filters. Headline finding: only **5 of 123** FASS producers land in WB top 100 (J.J. Prüm #5, Dönnhoff #7, Pierre Gonon #32, Jean Foillard #55, Peter Lauer #99) — FASS is NOT a path to fill WB-cellar gaps (Bedrock / Rhys / Ridge / Produttori / Dujac / Rousseau / Schaefer / Falkenstein / DRC all unavailable). What FASS IS: the deepest US source for several specialist niches. **16 FASS producers** got a WK topic title; top WK conviction goes to Foillard 40, Bel Air-Marquis d'Aligre 32, Château Thivin 15, Pierre Gonon 11, Daniel-Étienne Defaix 10. View structured as §1 triple-converge (the 5 WB matches), §2 heavy WK without WB (the FASS specialty zone — aged-classic Margaux, Côte-de-Brouilly, aged Chablis, Allemand-alum Cornas), §3 iconic by other measures (German biodynamic GG cluster Bürklin-Wolf / Christmann / Rebholz / Kühling-Gillot / Hermannsberg, Italian Nebbiolo AR.PE.PE. / Vietti, Loire Roches Neuves, Provence Château Simone, Savoie Berlioz, Jura Bourdy, grower Champagne Hébrart / Gimmonet / Moncuit / Callez-Lemaire / Sekthaus Raumland, Cahors Jouffreau 1986), §4 FASS's true structural depth (8 Cornas micro-domains, Côte-Rôtie natural cluster, aged-stock specialty Weber-Osterman 1989/1993 Auslese + Gilodi 1989 Bramaterra, Alto Piemonte sweep across Ghemme/Boca/Bramaterra/Fara/Sizzano/Carema, Graubünden Pinot), §5 skip (generic mid-tier Bordeaux, Brisset négoce, NV-only Champagne, Franken Bordeaux blends, Alexandrins). TL;DR ordering list at the bottom: Gonon both colors → Bel Air-Marquis d'Aligre verticals → Foillard case → AR.PE.PE. restock → Prüm Kabinett → Molitor pair → Bourdy Château-Chalon → Jouffreau 1986 → Lauer 1984 → Cuchet-Beliando Cornas.

## [2026-05-26] decisions | calibration: Östreicher full range + Brisset micro-négoce accepted

Two SKIP calls from `wiki/_views/fass_compelling_picks_2026_05.md` overturned by Evan's direct preference. (1) **Richard Östreicher's full range** (not just the Pinot Noir Katzenkopf) is accepted — Eichelmann Deutschlands Weine 2025 grants 4.5 stars and classifies the estate as "world-class winery" (Weltklasse); Falstaff 2025 awards 4 stars. The Cuvée "R" (Cab Sauv / Cab Franc / Merlot) — which I had framed as off-style for the Franken acceptance — is in fact the house's actual signature project, with Wein.plus describing the 2018 as "one of the most beautiful German red cuvées overall." Östreicher trained in Burgundy and brought back French plant material; his Spätburgunder rootstocks are original Burgundy clones. CLAUDE.md Franken filter updated to explicitly include the Bordeaux blend in-style. (2) **Pierre Brisset / Maison Brisset** accepted as in-style — I had framed it as generic Burgundy négoce. Wrong frame. It's a *micro-négoce* (founded 2014, capped at "few thousand bottles per year" total, exclusively 1er & GC, base at Château de Bligny, owns Chassagne vineyards). Outsider tier — Brisset has no vigneron lineage. Jancis Robinson MW covers him. CLAUDE.md curation taste updated to add "Burgundy micro-négoce (outsider tier)" as a new accepted category, with explicit gating criteria (micro production + exclusive 1er/GC focus + editorial coverage on the Jancis/Inside Burgundy/Vinous level). Producer pages `richard_ostreicher.md` and `pierre_brisset.md` rewritten with proper context; FASS compelling-picks view §5 SKIP section corrected with explicit "lesson learned" note (Falstaff/Eichelmann/Jancis-MW coverage outranks surface reading). TL;DR ordering list expanded from 10 to 15 with both producers in the top 5.

## [2026-05-26] ingest | fass: Tier 2 sweep + alias consolidation — 23 new pages, unmatched down to 18

Closed out the FASS unmatched list. Seeded **23 new producer pages** via `scripts/seed_fass_tier2.py`: 1 Tier 1 miss (Alain Burguet — Becky Wasserman discovery 1982, biodynamic since 2011, Mes Favorites VV from 18 parcels planted 1910s–1970s; should have been in tier 1), 1 Tier 1 upgrade after research (Jürgen von der Mark — **first German Master of Wine** 1996, Baden Spätburgunder), 9 Tier 2 with real prose (Vinding Montecarrubo — Peter Vinding-Diers's Sicily project on extinct volcano, Jancis-covered; La Psigula — new-gen Bramaterra; Cianfagna — Molise Aglianico; Pircher — Alto Adige Stadtberg Pinot; Louis Sozet — aged Cornas 2015/2016; Rattalino — Barolo/Barbaresco numbered Selections; Achim Dürr — Baden Pinot/Blaufränkisch/Cab Franc/Syrah; Weingut Riehen — Basel-Land Pinot + Le Cidre méthode traditionnelle), 4 Tier 3 stubs (Tenuta Col Falco, Tomaso Gianolo, Le Petit Château, Daniel & Monika Marugg), and 9 alias-consolidation roots for the split-row stragglers (calafe, domaine_gallet, luyton, marc_jambon, vini_marino, cantina_del_signore, chateau_de_trinquevedel, wegelin, max_geitlinger). Extended `FASS_ALIASES` in `ingest_fass.py` with 25 new entries covering spelling variants (Achim Dürr / Duerr / Weingut variants → single slug, etc.). Taxonomy patched to add Umbria + Molise as valid Italy regions. Pipeline: seed → ingest_csw (219 producers updated) → ingest_fass (146 matched, 18 unmatched) → build_rollups (47 region + 27 importer + 4 retailer pages, 30 producers with cellar bottles) → build_wiki_index. **FASS rollup: 14 → 146 producers across the rebuild arc.** The 18 remaining unmatched are intentional Tier 3 skips (4 generic mid-tier Bordeaux: Vieux St-Andrée / Samion / Bonneau / Bouillerot; 6 single-bottle obscure Italian: Cecllia Monte typo / Petra / Bellaria / Giuseppe Negro / Vigneti Costacurta / Domaine Michellas; 7 single-bottle obscure French: J. Despesse / JJ Morel / La Chablisienne / Raphael Chopin / Remy Nodin / Grosbot-Barbara / St. Gayan; 1 Spanish minor: Luna Beberide). Lint: 0 issues.
