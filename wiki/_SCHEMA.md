...
`raw/ratings/**/*.json` — currently `parse_auction_ratings.py` (auction catalog
`WineNote` fields) and `parse_raeders_ratings.py` (the Raeders `score_*` columns);
any future source in the same record shape merges in automatically. Distinct from
the *article*-shaped `## Vinous Reviews` / `## Wine Advocate (Kelley)` sections:
this is a compact **per-wine table of critic scores** with a short note excerpt
and per-source provenance, merged across sources per producer.
...
1. **Critic** — publication. From auction initials (`VM`→Vinous, `BH`→Burghound,
   `WS`→Wine Spectator, `WA`→Wine Advocate, `JS`→James Suckling, `JD`→Jeb Dunnuck,
   `IWC`→International Wine Cellar) or Raeders columns (`score_wa`→Wine Advocate,
   `score_js`→James Suckling, `score_w_s`→Wine Spectator, `score_we`→Wine
   Enthusiast). Unknown auction initials are kept verbatim; a bare `(NNpts)`
   renders `—`.
2. **Score** — preserves the source form (`97`, `94+`, `95-98`).
3. **Source** — `<sale>·W<week> lot <n>` (auction) or `Raeders <date>` for
   auditability.
